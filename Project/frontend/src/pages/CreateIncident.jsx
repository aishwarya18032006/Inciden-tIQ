...
const CreateIncident = () => {
 ...
 <h1 className="text-3xl font-black tracking-tight text-theme-text mb-2">Analyse Incident</h1>
 <p className="text-theme-text-sec dark:text-[#A1A1AA]">Follow the guided workflow to generate a multi-agent RCA report.</p>
 </div>
 ...
 <span className={`text-sm font-bold ${isActive ?'text-white' :'text-theme-text-sec dark:text-[#A1A1AA]'}`}>
 {step.num}
 </span>
 )}
 </motion.div>
 <span className={`absolute top-12 text-xs font-bold tracking-widest uppercase transition-colors duration-300 whitespace-nowrap ${isActive ?'text-blue-600 dark:text-[#A855F7]' :'text-theme-text-sec dark:text-[#71717A]'}`}>
 {step.title}
 </span>
 </div>
 );
 ...
 <label className="block text-xs font-bold text-theme-text-sec dark:text-[#A1A1AA] uppercase tracking-widest mb-2">Incident Title (Optional)</label>
 <input 
 type="text"
 value={title} 
 onChange={(e) => setTitle(e.target.value)} 
 className="w-full px-4 py-3 bg-theme-card dark:bg-[#111113] border border-theme-border dark:border-[#27272A] shadow-sm rounded-xl text-theme-text focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 dark:focus:border-[#A855F7] transition-all"
 ...
 <h2 className="text-2xl font-bold text-theme-text ">Timeline Information</h2>
 </div>
 <p className="text-theme-text-sec mb-8">Provide the sequence of events that occurred before and during the incident.</p>
 ...
 rows={8}
 className="w-full px-5 py-4 bg-theme-surface /50 border border-theme-border rounded-2xl text-theme-text-sec font-mono text-[13px] focus:outline-none focus:ring-4 focus:ring-blue-600/10 focus:border-blue-500 transition-all resize-none shadow-inner"
 ...
 <label className="cursor-pointer inline-flex items-center gap-2 text-[13px] font-bold text-theme-text-sec hover:text-blue-600 transition-colors uppercase tracking-wide">
 ...
 <h2 className="text-2xl font-bold text-theme-text ">Error Logs</h2>
 </div>
 <p className="text-theme-text-sec mb-8">Paste or upload application logs for AI analysis.</p>
 ...
 <button onClick={() => setActiveStep(1)} className="flex-1 sm:flex-none px-6 py-3.5 text-sm font-bold text-theme-text-sec hover:text-theme-text dark:hover:text-slate-200 transition-colors">Back</button>
 <button 
 ...
 <h2 className="text-2xl font-bold text-theme-text ">Recent Git Changes</h2>
 </div>
 <span className="px-3 py-1 bg-theme-bg border border-theme-border rounded-full text-[11px] font-bold text-theme-text-sec uppercase tracking-widest">Optional</span>
 </div>
 <p className="text-theme-text-sec mb-8">Paste recent code changes if available to help AI correlate deployment risks.</p>
 ...
 <button onClick={() => setActiveStep(2)} className="flex-1 sm:flex-none px-4 py-3.5 text-sm font-bold text-theme-text-sec hover:text-theme-text dark:hover:text-slate-200 transition-colors">Back</button>
 <button 
 onClick={() => handleSkip(3)}
 className="flex-1 sm:flex-none px-4 py-3.5 text-sm font-bold text-theme-text-sec hover:text-theme-text-sec transition-colors"
 >
 Skip
 </button>
 <button 
 ...
 <h2 className="text-3xl font-black text-theme-text tracking-tight mb-4">Ready For AI Analysis</h2>
 <p className="text-theme-text-sec max-w-md mx-auto mb-10 leading-relaxed text-[15px]">
 ...
 <p className="text-sm font-bold text-theme-text-sec uppercase tracking-widest animate-pulse">{loadingText}</p>
 </div>
 ) : (
 <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md justify-center">
 <button onClick={() => setActiveStep(3)} className="px-6 py-4 text-sm font-bold text-theme-text-sec hover:text-theme-text dark:hover:text-slate-200 transition-colors">
 Go Back
 </button>
 <button 
 onClick={generateRCA}
