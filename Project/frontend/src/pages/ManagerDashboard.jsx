...
const ManagerDashboard = () => {
 ...
 <div className="bg-theme-card border border-theme-border shadow-sm rounded-3xl p-8 relative overflow-hidden">
 <div className="max-w-2xl relative z-10">
 <div className="flex items-center gap-2 text-blue-600 mb-3">
 <ShieldAlert className="h-5 w-5"/>
 <span className="text-xs font-bold uppercase tracking-widest">IT Manager Workspace</span>
 </div>
 <h1 className="text-3xl font-black text-theme-text tracking-tight mb-4">Action Center</h1>
 ...
 <h2 className="text-xl font-bold text-theme-text ">Submitted Reports</h2>
 ...
 {loading ? (
 <div className="p-12 flex justify-center"><Activity className="h-6 w-6 text-theme-text-sec animate-spin"/></div>
 ) : reports.length === 0 ? (
 <div className="p-12 text-center flex flex-col items-center">
 <CheckCircle className="h-12 w-12 text-emerald-500 mb-4 opacity-50"/>
 <h3 className="text-lg font-bold text-theme-text mb-2">All Caught Up</h3>
 <p className="text-theme-text-sec ">There are no reports pending your review.</p>
 </div>
 ) : (
 <div className="overflow-x-auto">
 <table className="w-full text-left border-collapse">
 <thead>
 <tr className="border-b border-theme-border bg-theme-surface text-xs font-bold text-theme-text-sec uppercase tracking-widest whitespace-nowrap">
 ...
 {reports.map((report) => (
 <tr key={report.id} className="hover:bg-theme-surface dark:hover:bg-slate-800/50 transition-colors">
 <td className="px-6 py-4">
 <div className="font-bold text-theme-text mb-1">{report.submit_name || report.developer_name}</div>
 <div className="text-xs text-theme-text-sec ">{report.submit_email}</div>
 <div className="text-xs font-bold text-blue-600 mt-1">{report.submit_team}</div>
 </td>
 <td className="px-6 py-4">
 <div className="font-bold text-theme-text ">{report.incident_title ||'Unnamed Incident'}</div>
 <div className="text-xs text-theme-text-sec font-mono mt-1">Confidence: {report.confidence_score}%</div>
 </td>
 <td className="px-6 py-4">
...
'bg-amber-50 dark:bg-amber-900/20 text-amber-600 border-amber-200'
 ...
 <td className="px-6 py-4 text-sm text-theme-text-sec ">
 ...
 <Link to={`/reports/${report.incident_id}`} className="p-2 text-theme-text-sec bg-theme-bg hover:bg-slate-200 rounded-lg transition-colors"title="View">
 ...
 <h3 className="text-2xl font-black text-theme-text capitalize">{actionModal.type} Report</h3>
 </div>
 <div className="space-y-4 mb-8">
 <div>
 <label className="block text-sm font-bold text-theme-text-sec mb-2">Manager Comment (Optional)</label>
 <textarea 
 value={comment}
 onChange={(e) => setComment(e.target.value)}
 className="w-full px-4 py-3 bg-theme-surface border border-theme-border rounded-xl focus:ring-2 focus:ring-blue-500 font-medium text-theme-text "
 rows={4}
 ...
 <button onClick={() => setActionModal({ show: false, reportId: null, type: null})} className="flex-1 py-3 px-4 bg-theme-bg hover:bg-slate-200 text-theme-text-sec font-bold rounded-xl transition-colors">Cancel</button>
